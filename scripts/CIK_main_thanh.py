...
def main():
    ...
    for j in range(len(param['Ind_joint'])): 
            for i in range(1):
                ...
                x0 = np.append(x0,(model.lowerPositionLimit[j]+model.upperPositionLimit[j])/2)##np.append(x0,(model.lowerPositionLimit[j]+model.upperPositionLimit[j])/2)#np.append(x0,j)#
    starttime = time.time()
    ...
    # constraint bounds -> c == 0 needed -> both bounds = 0
        ...
        nlp.addOption('print_level', 1) # Tolerance on teh end-effector 3D position
        ...
        PEEd_iter=PEEd[[param['iter']-1,param['NbSample']+param['iter']-1,2*param['NbSample']+param['iter']-1]] 
