...
def main():
    ...
    R_all = np.empty(shape=(len(idx_meas), len(
        idx_param), NbSample+1), dtype=float)
    R_sub = np.zeros(shape=(len(idx_meas), len(
        idx_param), NbSample+1), dtype=float)
    ...
    for i in range(NbSample+1):
        ...
        model, data, R, J = Calculate_kinematics_model(
            q[:, i], model, data, IDX_TOOL)

        oMf_meas[:, i] = np.array(data.oMf[IDX_TOOL].translation)

        t = t+Ts

    oMf_meas_vect = np.reshape(
        oMf_meas[[idx_meas], :], (len(idx_meas)*(NbSample+1), 1))
    #print("New end-effector placement = ", data.oMf[IDX_TOOL])
    ...
    for iter in range(2):
        ...
        p = model.jointPlacements[1].translation
        model.jointPlacements[1].translation = p + \
            np.array([delta[0], delta[1], delta[2]])

        p = model.jointPlacements[2].translation
        model.jointPlacements[2].translation = p + \
            np.array([delta[4], delta[5], delta[6]])
        ...
        oMf_est_vect = np.reshape(
            oMf_est[[idx_meas], :], (len(idx_meas)*(NbSample+1), 1))
        J_reg = np.reshape(J_all.transpose(0, 2, 1),
                           (len(idx_meas)*(NbSample+1), 2))

        # for j, ind in enumerate(idx_meas):
        #    R_sub[j,:,:]=np.squeeze(R_all[ind,:,:])

        Regressor = np.reshape(R_all.transpose(
            0, 2, 1), (len(idx_meas)*(NbSample+1), len(idx_param)))
        ...
        # ,oMf_est_vect,'--r')
        plt.plot(oMf_meas_vect, 'k', oMf_est_vect, 'g--')
